...
class ApiClient
{
	...
	{
		$validDocumentTypes = array("FA", "NC", "ND", "FA_CONTINGENCIA", "FA_EXPORTACION", 
		...
		$client = $this->getClient();
		$request = $client->createRequest('POST', 'Issue/XML');
		$this->setAuthentication($request);
		$request->setHeader('X-REF-DOCUMENTTYPE',  $documentType);
		
		$this->setCustomHeader($request, $options, 'KeyControl', 'X-KEYCONTROL');
